...
public class ECC {
	...
	public static void main(String arg[]) throws Exception {
		...
		Random rnd = new Random(System.currentTimeMillis());
		BigInteger b1 = new BigInteger(ec.getP().bitLength(), rnd);
		BigInteger b2 = new BigInteger(ec.getP().bitLength(), rnd);
		Random r = new Random();
		int Low = 10;
		int High = 100;
		int factor = r.nextInt(High - Low) + Low;
		//reply = reply + "##$$" + b1 + "##$$" + b2 + "##$$" + factor;
		ECC ecc = new ECC();
		KeyPair keyPair = ecc.generateKeyPair2(ec, b1, b2);

		System.out.println(keyPair.getPublicKey().getKey());
		
		ec = EllipticCurve.NIST_P_192;
		ecc = new ECC();
		KeyPair keypair2 = ecc.generateKeyPair2(ec, b1, b2);
		System.out.println(keypair2.getPublicKey().getKey());
	}
}
